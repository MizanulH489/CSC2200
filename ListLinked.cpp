...
List<DataType>::ListNode::ListNode(const DataType& nodeData,
				   ListNode* nextPtr, ListNode* prevPtr)
...
{
    dataItem = nodeData; // set dataitem to be equal to nodeData
    next = nextPtr;  // set next to be equal to nexptr ,containing the elements
    prev = prevPtr; // create prev pointing to previous pointer 
}
...

{
    if ( isEmpty() ) {		// List is empty: could use "if ( cursor == 0 )"
		...
		ListNode *temp = new ListNode(newDataItem, cursor->next,cursor-> prev );
        cursor->next = temp;
        cursor = cursor->next;
        cursor->prev = temp;
...
{
    // If function for when the list is empty, throw a logic_error.
    if (isEmpty())
    {
        throw logic_error("remove-empty list"); // if item isempty, use throwlogic to print out empty list to screen
    }
    else
    {
        ...
        // Determine if the first element is the element to be removed.
        if (head == cursor)
        {
            nod_ptr = head->next;
            delete head;
        ...
        else // If the elemtn is not the first element, find it.
        {
            while (nod_ptr != NULL && nod_ptr != cursor)
            {
                previousNode = nod_ptr;
                nod_ptr = nod_ptr->next;
            }

            //if function to find node in link list
            if (nod_ptr)
            {
                previousNode->next = nod_ptr->next;
                cursor = previousNode;
                delete nod_ptr; // delete
...

{
    ...
    ListNode* t_1 = cursor;
    for (cursor = tail; cursor->prev != t_1; cursor = cursor->prev)
        /* Do nothing. All necessary work done in increment step above. */;
